...
### Web UI (`index.cgi`)
Runs as the DSM web server user (not root). Theme-aware layout: a status banner plus a compact chip row (VPN, routing, kill switch, auto-heal, forwarded port, Kuma, Transmission) that jumps to matching collapsible detail sections - traffic routing (v4 + v6 route/rule, blackhole state), kill switch, Kuma monitor, a Task Scheduler scripts quick reference, quick configuration, raw status, and a tail of `shield.log`. All checks use read-only commands that don't require root. It never reads `guard.secret`.
...
| `synology/scripts/guard-push` | Uptime Kuma push daemon (loop / once / final-down modes); reuses the shared `port_test()` cache |
...
- **UI**: the web status page (`index.cgi`) is redesigned - theme-aware (follows the browser's light/dark preference), the old six-card grid plus separate Kuma mini-grid is replaced by a compact clickable status-chip row that jumps to and opens the matching detail section, and detailed data (routing breakdown, kill switch, Kuma, raw status, log) now lives in collapsible sections instead of always-open cards. The page header shows the installed version and links to this documentation and GitHub, and a new Task Scheduler scripts section lists `activate`/`set-port`/`recover-heartbeat`/`recover-vpn` in-page. The long in-page configuration guide is trimmed to essentials, linking to the full docs instead of duplicating them.
